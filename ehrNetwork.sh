...
function printHelp() {
  echo "Usage: "
  echo "  ehrNetwork.sh <mode> [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-l <language>] [-o <consensus-type>] [-i <imagetag>] [-v]"
  ...
  echo "    -v - verbose mode"
  echo "  ehrNetwork.sh -h (print this message)"
  echo
  echo "Typically, one would first generate the required certificates and "
  echo "genesis block, then bring up the network. e.g.:"
  echo
  echo "	ehrNetwork.sh generate -c mychannel"
  echo "	ehrNetwork.sh up -c mychannel -s couchdb"
  echo "        ehrNetwork.sh up -c mychannel -s couchdb -i 1.4.0"
  echo "	ehrNetwork.sh up -l node"
  echo "	ehrNetwork.sh down -c mychannel"
  echo "        ehrNetwork.sh upgrade -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	ehrNetwork.sh generate"
  echo "	ehrNetwork.sh up"
  echo "	ehrNetwork.sh down"
}
...
# Versions of fabric known not to work with this release
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.ehrecords.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.ehrecords.com
    docker cp -a orderer.ehrecords.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.ehrecords.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.ehrecords.com

    for PEER in peer0.hospital1.ehrecords.com peer1.hospital1.ehrecords.com peer0.hospital2.ehrecords.com peer1.hospital2.ehrecords.com; do
...
function networkDown() {
  # stop org3 containers also in addition to hospital1 and hospital2, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/ehrecords-network --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/ehrecords-network/ledgers-backup
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/hospital1.ehrecords.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA_HOSPITAL1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/hospital2.ehrecords.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA_HOSPITAL2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
...
# and two Peer Orgs (``Hospital1`` & ``Hospital2``) each managing and maintaining two peer nodes.
...
# (``peer0.hospital1.ehrecords.com`` & ``peer0.hospital2.ehrecords.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Hospital1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Hospital1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/Hospital2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Hospital2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Hospital2MSP..."
    exit 1
  fi
  echo
}
...
CLI_DELAY=3
# channel name defaults to "ehrchannel"
